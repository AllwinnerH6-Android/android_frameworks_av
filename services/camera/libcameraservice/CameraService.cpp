...
Status CameraService::getNumberOfCameras(int32_t type, int32_t* numCameras) {
    ...
    char dev_node[16];
    status_t res;
    int32_t normalNumCameras;

    Mutex::Autolock l(mServiceLock);
    switch (type) {
        case CAMERA_TYPE_BACKWARD_COMPATIBLE:
            normalNumCameras = static_cast<int>(mNormalDeviceIds.size());
            ALOGV("%s:In CAMERA_TYPE_BACKWARD_COMPATIBLE.", __FUNCTION__);
            *numCameras = 0;
            ...
            for (int i = 0; i < 2; i++) {
                ...
                res = access(dev_node, F_OK);
                if(res == 0) {
                    (*numCameras)++;
                    status[i] = StatusInternal::PRESENT;
                } else {
                    status[i] = StatusInternal::NOT_PRESENT;
                }
            }
            if((*numCameras) != normalNumCameras) {
                ALOGD("%s:The old value:%d,the value checked video_node:%d,the value from HAL:%d.Camera number is wrong. Restart Camera HAL."
                                     ,__FUNCTION__,
                                     normalNumCameras,
                                     *numCameras,
                ...
                for (auto& cameraId : mCameraProviderManager->getCameraDeviceIds()) {
                    hardware::camera::common::V1_0::CameraResourceCost cost;
                    //The resource cost should update ,in this case ,maybe have some question.
                    ...
                    {
                        Mutex::Autolock lock(mCameraStatesLock);
                        std::shared_ptr<CameraState> state = std::make_shared<CameraState>(id8, cost.resourceCost, conflicting);
                        state->updateStatus(status[atoi(cameraId.c_str())], id8, {}, [](const String8& cameraId, StatusInternal status) {
                            ALOGD("%s:updateStatus,id:%s, Status:%d", __FUNCTION__, cameraId.c_str(),status);
                        });
                        mCameraStates.emplace(id8, state);
                        for (auto& i : mCameraStates) {
                            ALOGD("%s:mCameraStates,id:%s, Status:%d", __FUNCTION__, i.first.c_str(),
                                mapToInterface(i.second->getStatus()));
                            for (auto& listener : mListenerList) {
                                listener->onStatusChanged(mapToInterface(i.second->getStatus()),String16(i.first.c_str()));
                            }
                        }
                    }
                }
