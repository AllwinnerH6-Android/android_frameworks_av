...
#if SAVE_PCM
int DUMP_FLAG = 0;
#define PROPERTY_AUDIO_SOUNDCONTROL "audio.soundctrl.debug_flag"
const char* binpath = "/data/audio_d/soundctrl.pcm";
#endif
        ...
        if(sc->pAudioSink != NULL && !sc->needDirectOutPut)
        {
            ...
            */
            ...
            if(!sc->pAudioSink->ready()){
                logd("Cedarx use AudioSink open spr : %d, ch : %d, fmt : %u",
...
static int __SdWrite(SoundCtrl* s, void* pData, int nDataSize)
{
    ...
    if(DUMP_FLAG) {
        if(sc->testpcm) {
            if(nWritten >= 0) {
                fwrite(pData, 1, nWritten, sc->testpcm);
            }
        }
    }
#endif
...
SoundCtrl* SoundDeviceCreate(void* pAudioSink)
{
    ...
    property_get(PROPERTY_AUDIO_SOUNDCONTROL, val, "0");
    if (!strcmp(val, "0")) {
        DUMP_FLAG = 0;
    } else {
        DUMP_FLAG = 1;
    }
    logd("SoundControl DUMP_FLAG:%d", DUMP_FLAG);

    if(DUMP_FLAG) {
        s->testpcm = fopen(binpath,"w+");
        if(!s->testpcm)
        {
