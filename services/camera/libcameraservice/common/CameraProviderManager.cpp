...
int8_t CameraProviderManager::getNumberOfCameras() {
    int8_t ret_count = 0;
    for (auto& providerinfo : mProviders) {
                ret_count += providerinfo->getNumberOfCameras();
                ALOGV("%s: There are new getNumberOfCameras for '%s',ret_count: %d",
                __FUNCTION__, providerinfo->mProviderName.c_str(),ret_count);
    }
    return ret_count;
}
...
int8_t CameraProviderManager::ProviderInfo::getNumberOfCameras()  {
    ...
    numCameras = mInterface->getNumberOfCameras();
    ...
    if (!ret.isOk()) {
        ALOGE("%s: Transaction error in getting camera ID list from provider '%s'",
                __FUNCTION__, mProviderName.c_str());
        return DEAD_OBJECT;
    }
    if (status != Status::OK) {
    ...
    std::string id;
    CameraDeviceStatus newStatus = hardware::camera::common::V1_0::CameraDeviceStatus::PRESENT;

    for (auto& device : devices) {
        status_t res = addDevice(device,newStatus, &id);
        if (res != OK) {
            ALOGE("%s: Unable to enumerate camera device '%s': %s (%d)",
                    __FUNCTION__, device.c_str(), strerror(-res), res);
            continue;
    ...
    ALOGV("%s: Camera provider %s ready with %zu camera devices,the return value numCameras is %d.",
            __FUNCTION__,mProviderName.c_str(), mDevices.size(),numCameras);
