...
//for 4k recorder @30fps
    ...
    impl->m_sInPortDefType.format.video.eColorFormat = OMX_COLOR_FormatYUV420SemiPlanar; //fix it later
                                       ...
                                       OMX_INOUT OMX_PTR     pParamData)
{
    ...
    logv("get_parameter: eParamIndex: %x", (int)eParamIndex);
    ...
    switch (eParamIndex)
    {
        ...
        case OMX_IndexParamPortDefinition:
        {
                 ...
                 impl->m_sInPortDefType.nPortIndex)
            {
                ...
                logd(" get_parameter: width = %d, height = %d", (int)impl->m_sInPortDefType.format.video.nFrameWidth,
                     ...
                     impl->m_sOutPortDefType.nPortIndex)
            {
                ...
                logd(" get_parameter: width = %d, height = %d", (int)impl->m_sOutPortDefType.format.video.nFrameWidth,
        ...
        case OMX_IndexParamVideoPortFormat:
        {
            ...
            if (param_portformat_type->nPortIndex == impl->m_sInPortFormatType.nPortIndex)
            {
                ...
                {
                    loge("get_parameter erro: pParamData->nIndex > m_sInPortFormatType.nIndex,\
                                           ...
                                           OMX_IN OMX_PTR pParamData)
{
    ...
    logv("set_parameter: eParamIndex: %x", (int)eParamIndex);
    ...
    switch (eParamIndex)
    {
        case OMX_IndexParamPortDefinition:
        {
            ...
            if (((OMX_PARAM_PORTDEFINITIONTYPE *)(pParamData))->nPortIndex ==
                impl->m_sInPortDefType.nPortIndex)
            {
                logv("set_parameter: InPortIndex: %d",
                ...
                logd("init_input_port: stride = %d, width = %d, height = %d",
                     ...
                     impl->m_sOutPortDefType.nPortIndex)
            {
                logv("set_parameter: OutPortIndex: %d",
                     (int)((OMX_PARAM_PORTDEFINITIONTYPE *)(pParamData))->nBufferCountActual);
                if (((OMX_PARAM_PORTDEFINITIONTYPE *)(pParamData))->nBufferCountActual !=
                    impl->m_sOutPortDefType.nBufferCountActual)
                {
                ...
                logd("init_output_port: m_framerate: %d, output width %lu height %lu.", (int)impl->m_framerate,
                    impl->m_sOutPortDefType.format.video.nFrameWidth,
                                   ...
                                   OMX_IN OMX_PTR pConfigData)
{
    ...
    switch (eConfigIndex)
    {
        ...
        case OMX_IndexConfigVideoBitrate:
        {
            ...
            if (pData->nPortIndex == 1)
            {
                if (impl->m_state == OMX_StateExecuting && impl->m_encoder)
                {
                    ...
                    post_message_to_venc(impl, OMX_Venc_Cmd_ChangeBitrate);
                    logd("FUNC:%s, LINE:%d , pData->nEncodeBitrate = %lu",
        ...
        case OMX_IndexConfigVideoFramerate:
        {
            ...
            logd("FUNC:%s, LINE:%d. (Microphone)", __FUNCTION__,__LINE__);
            if (pData->nPortIndex == 0)
            {
                if (impl->m_state == OMX_StateExecuting && impl->m_encoder)
                {
                    ...
                    logv("FUNC:%s, LINE:%d , pData->xEncodeFramerate = %lu",
                                        ...
                                        OMX_IN    OMX_U32                nSizeBytes)
{
    ...
    OMX_S8                        nIndex = 0x0;
    OMX_PARAM_PORTDEFINITIONTYPE* pPortDef;
    logd("allocate buffer, nPortIndex: %s, nSizeBytes: %d", (int)nPortIndex == 0 ? "InPort":"OutPort",
        (int)nSizeBytes);
                                           ...
                                           OMX_IN OMX_BUFFERHEADERTYPE* pBufferHdr)
{
    ...
    logv(" vencOutPort: FUNC:%s, LINE: %d",__FUNCTION__,__LINE__);
            ...
            switch ((OMX_STATETYPE)(pCmdData))
            {
                ...
                case OMX_StateIdle:
                {
                    ...
                    else
                    {
                        ...
                        while (1)
                        {
                            logv("impl->m_sInPortDefType.bPopulated:%d, \
            ...
            if (ValidBitstreamFrameNum(impl->m_encoder) > 0)
            {
                ...
                if (impl->mOutBufHdr)
                {
                        ...
                        || impl->m_vencCodecType == VENC_CODEC_H265))
                    {
                        ...
                        if (OutFile)
                        {
                            fwrite(impl->mOutBufHdr->pBuffer, 1, impl->mOutBufHdr->nFilledLen, OutFile);
                        }
                        else
                        {
    ...
    while (1)
    {
        ...
        if(CdcMessageQueueTryGetMessage(impl->mqVencThread, &msg, 0) == 0)
        {
            ...
            switch (cmd)
            {
                ...
                {
                    logd("impl->m_framerate: %d, bitrate: %d", (int)impl->m_framerate,
        ...
        if (impl->m_state == OMX_StateExecuting && impl->m_encoder)
        {
            ...
            #if (OPEN_STATISTICS)
            nTimeUs2 = GetNowUs();
            logw("MicH264Enc, VideoEncodeOneFrame time %lld. buffer by encoder %ld and by decoder %ld.",
                (nTimeUs2-nTimeUs1), impl->m_outBufferCountOwnedByEncoder,
