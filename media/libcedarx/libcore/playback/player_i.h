...
enum EPLAYERINTERNALNOTIFY  //* player internal notify.
{
    ...
    PLAYER_VIDEO_RENDER_NOTIFY_10BIT_UNSUPPORT,
    ...
    PLAYER_VIDEO_RENDER_RESET_BUFFER_FINISHED,
};

// report every 5 seconds.
#define CONFIG_AUDIO_RENDER_TIME_DIFFERENCE_REPORT_PERIOD       (5000000)

// strategy for start or seek
// strategy 0: use the smaller first pts as the start time;
//            if audio first pts < video first pts, audio playback first;
//            if video first pts < audio first pts, video playback first;
// strategy 1: use the larger first pts as starttime;
//            if audio and video not sync, discard the unsync data, then start play;
// strategy 2: donot discard video data, timer start with video pts
//            if (a < v), discard some audio pts for sync;
//            if (a > v), video start first
