...
CdxParserT *CdxParserCreate(CdxStreamT *stream, cdx_uint32 flags)
{
    ...
    struct CdxParserNodeS *parserNode = NULL;
...
probe:
    ...
    if (psrNode)
    {
        ...
        {
            if(psrNode->type == CDX_PARSER_MP3)        //if mp3 suffix, then probe id3 if necessary.
            {
                logd("parser node maybe id3v2.");
                parserNode = malloc(sizeof(*parserNode));
                if(!parserNode){
                    loge("malloc id3v2 parser node failed.");
                    return NULL;
                }
                parserNode->creator = &id3v2ParserCtor;
                parserNode->type = CDX_PARSER_ID3V2;
                parserNode->keyInfo = &id3v2KeyInfo;
                if (parserNode->creator->probe(probeData) == 100)
                {
                    maxScoreNode = parserNode;
                    logd("parser node is id3v2.");
                    goto found;
                }
            }
        }
