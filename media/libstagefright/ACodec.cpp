...
status_t ACodec::configureCodec(
    ...
    if (haveNativeWindow) {
        ...
        // align in acodec in case of reallocate;
        int32_t width_1 = 0,height_1 = 0;
        if (!mIsEncoder
            && msg->findInt32("width", &width_1)
            && msg->findInt32("height", &height_1)) {
            ALOGD("ACodec configurecodec width=[%d],height=[%d]",width_1,height_1);
            const uint32_t mDecoderAlign = 32;// mEncoderAlign = 16;
            // IC(H6) decoder 32 align; encoder 16 align;
            // decoder align here better;
            // encoder align in encoder driver better;
            // uint32_t mAlign = mIsEncoder ? mEncoderAlign : mDecoderAlign;
            uint32_t mAlign = mDecoderAlign;
            width_1 = (width_1 + (mAlign - 1)) & (~(mAlign -1 ));
            height_1 = (height_1 + (mAlign - 1)) & (~(mAlign -1 ));
            ALOGI("ACodec align configurecodec mIsEncoder[%d],alignwidth=[%d],alignheight=[%d]",
                mIsEncoder, width_1,height_1);
        }
        msg->setInt32("width",width_1);
        msg->setInt32("height",height_1);
