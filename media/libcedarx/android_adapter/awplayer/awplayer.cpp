...
AwPlayer::AwPlayer()
{
    ...
    {
#if (CONF_ANDROID_MAJOR_VER < 9)
        //do not support HDCP in androidP
...
#endif
    sem_destroy(&mPriData->mReplyToDisconnect);
...
status_t AwPlayer::setDataSource(int fd, int64_t offset, int64_t length)
{
    ...
    else if (!strcmp(mPriData->strApkName, TVD_FILE_MANAGER_APP_NAME))
    {
        ...
        XPlayerSetScaleDown(mPriData->mXPlayer, 2);
    }
#endif
    else
        mPriData->mConfigInfo.appType = APP_DEFAULT;
...
//attention!!! Just for h5/h6, num(1,2,3,4) lock cpu num; num(5) lock freq.
