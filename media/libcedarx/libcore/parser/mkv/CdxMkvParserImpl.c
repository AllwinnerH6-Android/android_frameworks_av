                                 ...
                                 cdx_int32 offset)
{
    ...
    *size = (cdx_int32)rlength;
    ...
    // mkv track num is unique and not always sorted as 0, 1, 2 ...,etc.
    int sortedTrackNum = 0;

    for(int i = 0; i <  matroska->num_tracks; i++)
    {
        if(matroska->tracks[i]->num == track_num)
        {
            sortedTrackNum = i;
            break;
        }
        if(i == matroska->num_tracks)
        {
            loge("can't find track num of %d in stored %d matroska tracks!! ",
                track_num, matroska->num_tracks);
            return -1;
        }
    }
       ...
       matroska->tracks[matroska->VideoTrackIndex]->num == track_num)
    {
        ...
        else
        {
            ...
            matroska->tracks[sortedTrackNum]->type == MATROSKA_TRACK_TYPE_AUDIO ||
           matroska->tracks[sortedTrackNum]->type == MATROSKA_TRACK_TYPE_SUBTITLE )
    {
