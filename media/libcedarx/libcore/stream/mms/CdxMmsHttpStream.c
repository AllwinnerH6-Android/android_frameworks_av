...
int aw_mms_http_streaming_start(CdxDataSourceT* datasource, aw_mms_inf_t* mmsStreamInf)
{
   ...
   /*ASF_File_Properties_Object GUID and object size*/
    if(pos >= 0)
    {
        ...
        asfCtrl->packetSize = GetLE32((cdx_uint8*)&fileh->maxPacketSize);
        mmsStreamInf->fileSize  = GetLE64((cdx_uint8*)&fileh->fileSize);
        mmsStreamInf->fileDuration = (GetLE64((cdx_uint8*)&fileh->playDuration)/10000);
    }
