                ...
                if (mType == MIXER && !mStandby) {
                    ...
                    if (mThreadThrottle
                        ...
                        if ((signed)mHalfBufferMs >= throttleMs && throttleMs > 0) {
                            ...
                            /* In short,the underrun problem occus due to framesReady less than
                             ...
                             * Fix all underrun problem - 20181113
                             */
                            ALOGD("real mLackFrames = %d",mLackFrames);
                            if(mLackFrames > 0) { //只要检测到frameReady小于frameDesired则执行下方的延时等待
                                ...
                                #define MAX_TIME 5
                                do{
                                      usleep(1000);  //延时1毫秒,然后判断准备的数据是否足够,以及是否超时
                                      for(size_t i = 0; i < count; i++) {
                                          sp<Track> t = mActiveTracks[i];
                                          Track* const track = t.get();
                                          tmpReadyFrames = track->framesReady();
                                          minReadyFrames = tmpReadyFrames > minReadyFrames ? minReadyFrames : tmpReadyFrames;
                                          ALOGD("minReadyFrames=%d, mNeededFrames=%d",minReadyFrames, mNeededFrames);
                                      }
                                }while((minReadyFrames < mNeededFrames) && (waitTimeout++ < MAX_TIME));  //如果数据不足，且没有超时则继续等待
                            }
                            mLackFrames = 0;
                            minReadyFrames = 0;
