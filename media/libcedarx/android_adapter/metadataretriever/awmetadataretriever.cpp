...
#else // android_version >= 8.1
status_t AwMetadataRetriever::setDataSource(const sp<DataSource>& source, const char *mime)
#endif

{

#if ((CONF_ANDROID_MAJOR_VER == 8) && (CONF_ANDROID_SUB_VER > 0)) || (CONF_ANDROID_MAJOR_VER >8)
    CEDARX_UNUSE(mime);
#endif
...
//if we get the title name fail,resolve the title from the file name.
    char *FilePath;
    if(kMap[10].name)
    {
        char *ptr;
        char c = '/';
        FilePath = FdToFilepath(mPriData->mFd);
        ptr = strrchr(FilePath,c);
        if(ptr)
        {
            ptr++;
            detector->addTag(kMap[10].name, (const char*)ptr);
        }
    }
...
#else
sp<IMemory> AwMetadataRetriever::getFrameAtTime(
            int64_t timeUs, int option, int colorFormat, bool metaOnly)
{
    ...
    VideoFrame* pVideoFrame = new VideoFrame(pXVideoFrame->mWidth, pXVideoFrame->mHeight,
            ...
            pXVideoFrame->mRotationAngle, bpp, !metaOnly, 0/*iccSize*/);
    if(pVideoFrame == NULL)
    {
        loge("failed to new VideoFrame");
        return NULL;
    }

    size_t size = pVideoFrame->getFlattenedSize();
    ...
    if (frameMem == NULL)
    {
        loge("not enough memory for VideoFrame size=%zu", size);
        return NULL;
    }
    VideoFrame* frameCopy = static_cast<VideoFrame*>(frameMem->pointer());
    frameCopy->init(*pVideoFrame, NULL, 0);

    memcpy(frameCopy->getFlattenedData(), pXVideoFrame->mData, pXVideoFrame->mSize);
    return frameMem;
}
sp<IMemory> AwMetadataRetriever::getImageAtIndex(
            int index, int colorFormat, bool metaOnly, bool thumbnail)
{
    CDX_UNUSE(index);
    CDX_UNUSE(thumbnail);
    CDX_UNUSE(colorFormat);
    CDX_UNUSE(metaOnly);
    logd("++++ unsupport function getImageAtIndex");
    return NULL;
}
sp<IMemory> AwMetadataRetriever::getImageRectAtIndex(
            int index, int colorFormat, int left, int top, int right, int bottom)
{
    ...
    CDX_UNUSE(right);
    CDX_UNUSE(bottom);
    logd("++++ unsupport function getImageRectAtIndex");
    return NULL;
}
status_t AwMetadataRetriever::getFrameAtIndex(
            std::vector<sp<IMemory> >* frames,
            int frameIndex, int numFrames, int colorFormat, bool metaOnly)
{
    CDX_UNUSE(frames);
    CDX_UNUSE(frameIndex);
    CDX_UNUSE(colorFormat);
    CDX_UNUSE(numFrames);
    CDX_UNUSE(metaOnly);
    logd("++++ unsupport function getFrameAtIndex");
    return 0;
}
