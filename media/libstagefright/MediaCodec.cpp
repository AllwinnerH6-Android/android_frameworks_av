...
status_t MediaCodec::configure(
    ...
    if (mIsVideo) {
        ...
        // for media debug.
        ALOGD("Debug mediacodec delay, you can: setprop debug.media.mediacodec_delay 1 ");
        char prop_value[PROPERTY_VALUE_MAX];
        property_get("debug.media.mediacodec_delay", prop_value, "0");
        mRecordDelay = (atoi(prop_value) == 1) ? true : false;
...
static int64_t  MediaCodec_getNowUs() {
    struct timeval now;
...
status_t MediaCodec::queueInputBuffer(
    ...
    if (mRecordDelay && mIsVideo ) {
        TraceBitStreamToFrameDelay  mTraceB2FDelay;
        mTraceB2FDelay.mQueueInputputBufferT = MediaCodec_getNowUs();
        mMapB2FDalay[presentationTimeUs/1000] = mTraceB2FDelay;
        ALOGD("DELAY: %s queueInputBuffer idx = %d, pts = %lld",
              ((mFlags & kFlagIsEncoder) ? "Encoder" : "Decoder"),
              index,presentationTimeUs/1000);
    }
...
status_t MediaCodec::dequeueOutputBuffer(
    ...
    if (mRecordDelay && mIsVideo) {
        TraceBitStreamToFrameDelay  mTraceB2FDelay = mMapB2FDalay[*presentationTimeUs/1000];
        mTraceB2FDelay.mDeQueueOutputBufferT = MediaCodec_getNowUs();
        mMapB2FDalay[*presentationTimeUs/1000] = mTraceB2FDelay;
        ALOGV("DELAY: %s dequeueOutputBuffer idx = %d, pts = %lld ms, qIn2DqOut delay = %lld us",
             ((mFlags & kFlagIsEncoder) ? "Encoder" : "Decoder"),
              *index,
              *presentationTimeUs/1000,
              mTraceB2FDelay.mDeQueueOutputBufferT - mTraceB2FDelay.mQueueInputputBufferT);
    }
...
size_t MediaCodec::updateBuffers(
    ...
    if (mRecordDelay  && mIsVideo && portIndex == kPortIndexOutput) {
        int64_t mediaTimeUs = -1;
        buffer->meta()->findInt64("timeUs", &mediaTimeUs);
        TraceBitStreamToFrameDelay	mTraceB2FDelay = mMapB2FDalay[mediaTimeUs/1000];
        mTraceB2FDelay.mOutputBufferAvaliT = MediaCodec_getNowUs();
        mMapB2FDalay[mediaTimeUs/1000] = mTraceB2FDelay;
        ALOGV("DELAY: %s onOutputBufferAvailable idx = %d, pts = %lld ms, quIn2OutAvali delay = %lld us",
              ((mFlags & kFlagIsEncoder) ? "Encoder" : "Decoder"),
              index,
              mediaTimeUs/1000,
              mTraceB2FDelay.mOutputBufferAvaliT - mTraceB2FDelay.mQueueInputputBufferT);
    }
...
status_t MediaCodec::onReleaseOutputBuffer(const sp<AMessage> &msg) {
    ...
    if (render && buffer->size() != 0) {
        ...
        if (mRecordDelay && mIsVideo) {
            TraceBitStreamToFrameDelay  mTraceB2FDelay = mMapB2FDalay[mediaTimeUs/1000];
            mTraceB2FDelay.mReleaseOutputBufferT = MediaCodec_getNowUs();
            mMapB2FDalay[mediaTimeUs/1000] = mTraceB2FDelay;
            mMapB2FDalay.erase(mediaTimeUs/1000);
            int64_t mQIn2DqOut =  (mFlags & kFlagIsAsync) ?
                        mTraceB2FDelay.mOutputBufferAvaliT - mTraceB2FDelay.mQueueInputputBufferT :
                        mTraceB2FDelay.mDeQueueOutputBufferT - mTraceB2FDelay.mQueueInputputBufferT;
            int64_t mQIn2Avali = mTraceB2FDelay.mOutputBufferAvaliT - mTraceB2FDelay.mQueueInputputBufferT;
            ALOGD("DELAY: %s %s onReleaseOutputBuffer idx = %d, pts = %lld ms"
                  "qIn2-[OutAvali DqOut Render] delay us [Delay %s] = %lld, %lld, %lld",
                  ((mFlags & kFlagIsEncoder) ? "Encoder" : "Decoder"),
                  ((mFlags & kFlagIsAsync)   ? "Async"   : "Sync   "),
                  index,
                  mediaTimeUs/1000,
                  (mQIn2Avali > 15000 ? "Toolong":"Normal"), // delay > 15ms;
                  mQIn2Avali,
                  mQIn2DqOut,
                  mTraceB2FDelay.mReleaseOutputBufferT - mTraceB2FDelay.mQueueInputputBufferT);
       }
